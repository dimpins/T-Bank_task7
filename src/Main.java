...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < list.size(); i++) {
            ...
            for(int j = i+1; j < list.size(); j++) {
                if(a == list.get(j)) {
                    ...
                    sameNumCount++;
                    if(sameNumber.size() > 1 || sameNumCount > 1) {    //если повторяющихся чисел больше 1, или нашло повторяющееся, но последний элемент был изменён, то не получится
                        System.out.println("-1 -1");
                        return;
                    }
                }
        ...
        if(sameIndexAndNumCount == 1) {         //если число совпадает со своим индексом
        ...
        } else if(sameNumber.size() == 1) {         //если повторяющееся число
            ...
            if(list.indexOf(sameNumber.getFirst()) + 1 == missingNum){               //если индекс, где впервые встречается повторяющееся число, совпадает с пропущенным числом
    ...
    private static int findMissingNumber(ArrayList<Integer> list) {          //поиск пропущенного числа
        ...
        int missingNum = 0;
        for(int i = 0; i < list.size(); i++) {
            if(!sortedList.contains(i+1)){       //если число не совпало со своим индексом, то пропущенный элемент индекс
                missingNum = i+1;
                break;
            }
        }
